...
const store = async (req, res, next) => {
    try{
        ...
        //update untuk relasi dengan tag (one-to-many)
...
const update = async (req, res, next) => {
    try{
        ...
        if(req.file){
           ...
           src.on('end' , async () => {
               try {
                   //fungsi untuk menghapus file gambar lama dan mengganti dengan yang baru
...
const index = async (req, res, next) => {
    try {
        let { skip = 0, limit =0 , category = '' , q='' , tags=[]} = req.query;
        let criteria = {};
        if(q.length){
            criteria = {
                ...criteria,
